...
## macOS 开发如何测试
由于 AHK/EXE 运行环境是 Windows，macOS 下建议这样做：

1. 日常开发（macOS）
- 修改 `haloCaps.ahk`、`conf.ini` 和库文件，提交代码。

2. 自动编译测试（GitHub Actions）
- 仓库已提供 Windows Runner 的自动编译流程，提交到 `master` 后会自动编译。
- 可以在 Actions 页面查看编译日志和产物。

3. 功能验证（需要 Windows）
- 要验证真实按键行为（例如 `CapsLock`、`!v` 连续粘贴），需在 Windows 实机或虚拟机中运行测试。

## 自动化发布（master 分支）
已新增工作流：`.github/workflows/release-master.yml`
...
流程内容：
1. 在 `windows-latest` 下载 AutoHotkey v2
2. 编译 `haloCaps.ahk` -> `dist/haloCaps.exe`
3. 打包 `haloCaps-master-<run_number>.zip`
4. 自动创建 GitHub Release（tag: `master-<run_number>`）并上传构建产物
